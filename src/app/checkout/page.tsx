...
export default function CheckoutPage() {
  ...
  function handleConfirmAndSend() {
    ...
    // 1. Build WhatsApp URL synchronously — direct user click, no popup blocker
    const url = buildWhatsAppUrl(payload);
    window.open(url, "_blank");

    // 2. Clear cart and show confirmation immediately
    clear();
    setStep("confirmed");

    // 3. Save to Supabase in the background — doesn't block UX
    saveOrder(payload);
  }
                ...
                <div className="p-6 flex flex-col gap-4">
                  <button onClick={() => setPaymentMethod("paystack")}
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <BodyMd className="text-[13px]">Card, bank, USSD, or mobile money</BodyMd>
                      </div>
                  ...
                  {formError && (
                    <div className="flex items-center gap-2 bg-error-container text-on-error-container rounded-lg px-4 py-3">
                      ...
                      <BodyMd className="text-[13px]">{formError}</BodyMd>
                    </div>
                  )}
                  ...
                  {paymentMethod && (
                    <button
                      ...
                      className="w-full mt-2 flex items-center justify-center gap-3 bg-[#25d366] text-white rounded-full px-8 py-4 font-body font-bold text-[16px] hover:brightness-95 active:scale-95 transition-all shadow-raised cursor-pointer"
                    >
